...
describe("connection & auth", () => {
    ...
    it("rejects invalid token", async () => {
        const ws = new WebSocket(`ws://127.0.0.1:${server.port}?token=bad-token`);
...
describe("session lifecycle", () => {
    it("lists sessions (initially empty)", async () => {
        ...
        expect(Array.isArray(msg.payload.sessions)).toBe(true);
        ws.close();
    });
    it("creates a session and lists it", async () => {
        ...
        expect(created.payload.name).toBe("test-session");
    ...
    it("attaches to a session", async () => {
        ...
        expect(attached.payload.target).toBe(sessionId);
        ws.close();
    });
    it("detaches from a session", async () => {
        ...
        expect(detached.payload.reason).toBe("client_request");
        ws.close();
    });
    it("errors when attaching to non-existent session", async () => {
        ...
        expect(err.payload.code).toBe("SESSION_NOT_FOUND");
        ws.close();
    });
    it("errors on input when not attached", async () => {
        ...
        const err = await waitForMsg(ws, "error");
        expect(err.payload.code).toBe("NOT_ATTACHED");
        ws.close();
    });
...
describe("resize: host dictates size", () => {
    it("session starts with the creation size", async () => {
        ...
        expect(attached.payload.cols).toBe(120);
        expect(attached.payload.rows).toBe(40);
        host.close();
    });
    it("host resize → all clients notified", async () => {
        ...
        expect(webAttached.payload.cols).toBe(80);
        expect(webAttached.payload.rows).toBe(24);
        ...
        expect(hostResized.payload.cols).toBe(160);
        expect(hostResized.payload.rows).toBe(48);
        const webResized = await webResizePromise;
        expect(webResized.payload.cols).toBe(160);
        expect(webResized.payload.rows).toBe(48);
        host.close();
        web.close();
    });
    ...
    it("host resize after web joins → web gets new size", async () => {
        ...
        const webResized = await webResizePromise;
        expect(webResized.payload.cols).toBe(200);
        expect(webResized.payload.rows).toBe(60);
        host.close();
        web.close();
    });
    it("multiple host resizes → web tracks each one", async () => {
        ...
        expect(webResized.payload.cols).toBe(120);
        // Second resize
        ...
        webResized = await webResizePromise;
        expect(webResized.payload.cols).toBe(60);
        expect(webResized.payload.rows).toBe(20);
        host.close();
        web.close();
    });
});
